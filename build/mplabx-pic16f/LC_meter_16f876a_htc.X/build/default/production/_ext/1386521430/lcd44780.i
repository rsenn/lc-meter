...
# 52 "C:\Program Files (x86)\Microchip\xc8\v2.10\pic\include\pic16f876a.h"
...
uint8_t i;
for(i = 0; string[i]; i++) lcd_send((string[i]), 1);
}
...
while(pad-- >= i) lcd_send((padchar), 1);

for(; i > 0; i--) lcd_send(((char)buf[(int16_t)i - 1]), 1);
