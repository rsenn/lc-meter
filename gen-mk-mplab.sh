...
[ $# -gt 0 ] && SOURCES="$*" ||
SOURCES='${PROGRAM_NAME}.? lib/{comparator,format,random,ser,softpwm,timer,uart}.? src/config-${CHIP}.h src/config-bits.h src/*.c'
    ...
    for BUILD_TYPE in $BUILD_TYPES; do
      for COMPILER in $COMPILERS; do
       (eval "SOURCES=\$(set -- $SOURCES; echo \$(ls -d \$@) 2>/dev/null)"
        echo "SOURCES='$SOURCES'" 1>&2
        eval "(set -x;	genmakefile -t "$COMPILER" -m mplab $@ -I. -Ilib -Isrc "$SOURCES" -DHAVE_COMPARATOR=1 -DSER_TIMER=2 -DUART_BAUD=38400 -DUSE_SOFTPWM=1 -DUSE_SER=1 -DUSE_TIMER0=1 -DUSE_TIMER1=1 -DUSE_TIMER2=1 -D_XTAL_FREQ=20000000 --create-bins --no-create-libs --$BUILD_TYPE --chip=${CHIP} -d obj/${COMPILER}-${CHIP}/$BUILD_TYPE/ -o build/mplab/$PROGRAM_NAME-$CHIP-$COMPILER-$BUILD_TYPE.mcp >&genmakefile-$COMPILER-$BUILD_TYPE.log) 2>&10")
      done
    done
  done
done
