...
measure_freq() {
  ...
  do {
    ...
    NOP();
    NOP();
    counter++; // count until TMR0 incremented
  } while (prev_tmr0 == TMR0 &&
           counter <= 255); // test if timer0 has incremented

  //}while(prev_tmr0==TMR0);  //test if timer0 has incremented
  counter = ((prev_tmr0 << 8) + (256 - counter));
/*
#if USE_SER
  format_number(ser_putch, counter, 10, 0);
#endif
*/
  return counter;
}
...
/*
 * Delay using milliseconds counter
 */
void
