...
lcd_putchar(char c) {
  c;
  _asm mov b, dpl mov dptr,
#LCD_STATUS_RD 00001 $ : movx a, @dptr jb acc .7, 00001 $ mov a, b mov dptr, #LCD_DATA_WR movx @dptr, a _endasm;
}

void
lcd_command(void) /* actually takes command byte in b */
{
  _asm mov dptr, #LCD_STATUS_RD 00001 $ : movx a,
                                          @dptr jb acc .7,
                                          00001 $ mov a,
                                          b mov dptr,
#LCD_COMMAND_WR movx @dptr,
                                          a _endasm;
}

void
lcd_init(void) {
  _asm mov b, #LCD_CONFIG_CMD lcall _lcd_command mov b, #LCD_ON_CMD lcall _lcd_command mov b,
#LCD_SHIFT_CMD lcall _lcd_command _endasm;
}
...
lcd_set_xy_raw(unsigned int x_and_y) {
  ...
  _asm;
  check X is within display size mov a, dpl add a, #256 - LCD_HORIZ_SIZE jnc 00001 $ mov dpl,
#LCD_HORIZ_SIZE - 1 00001 $:;
      check Y is within display size mov a, dph add a, #256 - LCD_VERT_SIZE jnc 00002 $ mov dph,
#LCD_VERT_SIZE - 1 00002 $:;
      compute address within data display ram in LCD controller chip mov a, dph mov b, #LCD_BYTES_PER_LINE mul ab add a,
      dpl orl a, #0x80;msb set for set DD RAM address
	mov	b, a
	lcall	_lcd_command
	_endasm;
}
...
void
lcd_clear(void) {
  _asm mov b, #LCD_CLEAR_CMD lcall _lcd_command _endasm;
}

void
lcd_home(void) {
  _asm mov b, #LCD_HOME_CMD lcall _lcd_command _endasm;
}
