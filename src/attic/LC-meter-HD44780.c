...
//#pragma config WDTE = OFF, PWRTE = ON, CP = OFF, BOREN = ON, DEBUG = OFF, LVP
//= OFF, CPD = OFF, WRT = OFF, FOSC = HS
#endif
...
float measure_freq(void) { // 16-bit freq
...
float measure_freq_tmr1(void) { // 16-bit freq
  TRISC0 = INPUT;
  TRISA4 = 0; // Enable RA4 output to T0CKI
